...
const boards = inputArr.slice(1).map(board => board.split("\n").map(line => line
    ...
    .split(" ")
    .map(x => Number(x))));
const checkIfBoardWins = (board) => {
    for (let i = 0; i < board.length; i++) {
        if (board[i].reduce((a, b) => a + b) === board.length * -1 ||
            board.reduce((acc, line) => acc + line[i], 0) === board.length * -1)
            return true;
    }
    return false;
};
const solveBoard = (numbers, board) => {
    let solvedBoard = [...board.map(line => [...line])];
    let turnIndex = 0;
    for (let i = 0; i < numbers.length; i++) {
        solvedBoard.forEach(line => line.forEach((num, index) => {
            if (num === numbers[i])
                line[index] = -1;
        }));
        turnIndex = i;
        if (checkIfBoardWins(solvedBoard))
            break;
    }
    return { turnIndex, solvedBoard };
};
const getSumOfUnmarkedNums = (board) => {
    return board.flat().reduce((a, b) => a + (b === -1 ? 0 : b), 0);
};
const solver = (numbers, boards, firstOrLast) => {
