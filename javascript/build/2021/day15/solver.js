...
const solver = () => {
    ...
    const getNeighboursFromPoint = (x, y) => {
        let neighbours = [];
        ...
        if (x !== gridHeight - 1)
            neighbours.push([x + 1, y]); // bottom
        if (y !== gridWidth - 1)
            neighbours.push([x, y + 1]); // right
        return neighbours;
    };
    ...
    const fillHeightMap = (currentLevel) => {
        const findMinimumRisk = (x, y, direction) => {
            ...
            if (direction === "row") {
                for (let i = 0; i < gridWidth - y; i++) {
                    ...
                    minimumRisk = Math.min(minimumRisk, currentRisk + heightMap[x + 1][y + i]);
                }
            }
                ...
                for (let i = 0; i < gridWidth - x; i++) {
                    ...
                    minimumRisk = Math.min(minimumRisk, currentRisk + heightMap[x + i][y + 1]);
                }
            }
        ...
        heightMap[currentLevel][currentLevel] =
            inputArr[currentLevel][currentLevel] +
                Math.min(heightMap[currentLevel + 1][currentLevel], heightMap[currentLevel][currentLevel + 1]);
    };
