...
const solver2 = (heightMap) => {
    let basinSizes = [];
    let mapCopy = heightMap.map(row => [...row]);
    const findBasinFromOrigin = (x, y) => {
        let basinSize = 0;
        const findNeighboursFromPoint = (x, y) => {
            ...
            for (const neighbour in neighbours) {
                if (neighbours[neighbour].value !== 9 &&
                    neighbours[neighbour].value !== "x") {
                    findNeighboursFromPoint(neighbours[neighbour].xPos, neighbours[neighbour].yPos);
                }
            }
            return;
        };
        ...
        mapCopy.forEach((row, i) => (mapCopy[i] = row.map(value => {
            if (value === "x") {
                basinSize++;
                return 9;
            }
