...
const inputArr: string[] = readFile("2015", "day5").toString().split("\n");
...
const part1Solver = () => {
  ...
  inputArr.forEach(inputString => {
    ...
    for (let i = 0; i < paddedInputString.length - 1; i++) {
      ...
      if (char === nextChar) hasConsecutiveDuplicates = true;
      if (vowels.includes(char)) numVowels++;
    }
    if (
      numVowels >= 3 &&
...
const part2Solver = () => {
  ...
  inputArr.forEach(inputString => {
    ...
    const pairAppearanceNum: Record<string, number> = {};

    const paddedInputString = `${inputString}.,`;

    for (let i = 0; i < paddedInputString.length - 2; i++) {
      const hasValidPair = Object.values(pairAppearanceNum).some(
        val => val > 1
      );
      ...
      if (char === nextNextChar) hasRepeatLettersWithGap = true;
