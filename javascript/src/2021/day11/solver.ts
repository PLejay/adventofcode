import { getGridDimensions, printGrid, readFile } from "../../utils/utils";
...
const inputArr: number[][] = readFile("2021", "day11")
  .toString()
  .split("\n")
...
const solver = (initialValues: number[][], stepGoalNumber = 100) => {
  ...
  const getNeighbourCoordinates = (
    ...
    if (x !== 0) {
      ...
      if (y !== 0) availableNeighbourCoords.push({ x: x - 1, y: y - 1 }); // top left
      if (y !== gridWidth - 1)
        availableNeighbourCoords.push({ x: x - 1, y: y + 1 }); // top right
    }
    if (y !== 0) availableNeighbourCoords.push({ x, y: y - 1 }); // left
    if (y !== gridWidth - 1) availableNeighbourCoords.push({ x, y: y + 1 }); // right
    if (x !== gridHeight - 1) {
      availableNeighbourCoords.push({ x: x + 1, y }); // bottom
      if (y !== gridWidth - 1)
        availableNeighbourCoords.push({ x: x + 1, y: y + 1 }); // bottom right
      if (y !== 0) availableNeighbourCoords.push({ x: x + 1, y: y - 1 }); // bottom left
    }
  ...
  const step = (stepNumber: number) => {
    const raiseEnergy = (point: Coordinates) => {
      const { x, y } = point;
