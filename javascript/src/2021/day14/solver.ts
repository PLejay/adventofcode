...
type UpdatedInstruction = {
  pair: string;
  charToInsert: string;
  createdPairs?: string[];
};

const inputArr: string[] = readFile("2021", "day14").toString().split("\n\n");

const startingTemplate: string = inputArr[0];
const instructions: string[][] = inputArr[1]
  .split("\n")
...
const listFrequency = (polymer: string) => {
  let frequencyMap: { [char: string]: number } = {};
...
const updateInstructionsWithRelevantPairs = (): UpdatedInstruction[] => {
  let updatedInstructions: UpdatedInstruction[] = [];
  instructions.forEach(inst => {
    let updatedInstruction: UpdatedInstruction = {
      pair: inst[0],
      charToInsert: inst[1],
    };
...
const solver = (stepNum = 40) => {
  ...
  const sortedCharFrequency = Object.entries(updatedStats.charFrequency).sort(
    (a, b) => a[1] - b[1]
  );

  return (
    sortedCharFrequency[sortedCharFrequency.length - 1][1] -
