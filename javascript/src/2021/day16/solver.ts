...
const hexToBinary = (hexValue: string): string | void => {
  const hexValueMap: {
    [index: string]: string;
  } = {
    "0": "0000",
...
const inputString: string = readFile("2021", "day16")
  .toString()
  .split("")
...
type PacketRule = {
  type: "totalBitLength" | "subPacketNum";
  value: number;
};

const analyseParentPacket = (packets: string) => {
  ...
  let lengthTypeId: string | null = null;
  let childPacketsRule: PacketRule | null = null;
  let childPackets: string | null = null;
  let remainingPackets: string | null = null;
...
const solver = () => {
  ...
  let currentRules = [];
  const decodePackets = (packets: string) => {
    console.log(packets);
    const {
      versionNum,
      typeId,
    ...
    if (!isOperator) {
      ...
      let totalPacketLength =
        packets.length - (childPackets ? childPackets.length : 0);
      ...
      while (
        ...
        const { remainingPackets, packetLength } = decodePackets(
          currentRemainingPackets
        );
        childPacketNum++;
